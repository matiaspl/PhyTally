...
static inline bool _init_async_event_queue(){
    if(!_async_queue){
        // Under concurrent HTTP responses on ESP32-S3 USB-NCM, the default
        // queue depth is too small and lwIP callbacks end up blocking on
        // xQueueSend(portMAX_DELAY), stalling the TCP/IP thread. Keep the
        // async handoff queue comfortably above small browser burst sizes.
...
static err_t _tcp_new_api(struct tcpip_api_call_data *api_call_msg){
    ...
    msg->err = ERR_OK;
    msg->pcb = tcp_new_ip_type(msg->ip_type);
    if (!msg->pcb) {
        msg->err = ERR_MEM;
    }
    return msg->err;
...
static tcp_pcb * _tcp_new_ip_type(u8_t ip_type) {
    tcp_api_call_t msg;
    msg.pcb = NULL;
    msg.closed_slot = -1;
    msg.ip_type = ip_type;
...
static err_t _tcp_accept_setup_api(struct tcpip_api_call_data *api_call_msg){
    tcp_api_call_t * msg = (tcp_api_call_t *)api_call_msg;
    msg->err = ERR_OK;
    tcp_arg(msg->pcb, msg->arg);
    tcp_accept(msg->pcb, &AsyncServer::_s_accept);
    return msg->err;
}

static esp_err_t _tcp_accept_setup(tcp_pcb * pcb, void * arg) {
    if(!pcb){
        return ESP_FAIL;
    }
    tcp_api_call_t msg;
    ...
    msg.closed_slot = -1;
    msg.err = ERR_OK;
    tcpip_api_call(_tcp_accept_setup_api, (struct tcpip_api_call_data*)&msg);
    return msg.err;
}

static err_t _tcp_backlog_accepted_api(struct tcpip_api_call_data *api_call_msg){
    tcp_api_call_t * msg = (tcp_api_call_t *)api_call_msg;
    msg->err = ERR_OK;
    tcp_backlog_accepted(msg->pcb);
    return msg->err;
}

static esp_err_t _tcp_backlog_accepted(tcp_pcb * pcb) {
    if(!pcb){
        return ESP_FAIL;
    }
    tcp_api_call_t msg;
    ...
    msg.closed_slot = -1;
    msg.err = ERR_OK;
    tcpip_api_call(_tcp_backlog_accepted_api, (struct tcpip_api_call_data*)&msg);
    return msg.err;
}
...
bool AsyncClient::_connect(ip_addr_t addr, uint16_t port){
    ...
    tcp_pcb* pcb = _tcp_new_ip_type(IPADDR_TYPE_ANY);
    if (!pcb){
        log_e("pcb == NULL");
        return false;
    }
...
void AsyncServer::begin(){
    ...
    _pcb = _tcp_new_ip_type(bind_type);
    if (!_pcb){
        log_e("_pcb == NULL");
        return;
    }
...
int8_t AsyncServer::_accept(tcp_pcb* pcb, int8_t err){
    ...
    if(_connect_cb){
        ...
        if(c){
            ...
            // Re-open one listen backlog slot now that lwIP has handed off the
            // accepted pcb to AsyncTCP. Without this, the listen backlog is
            // consumed permanently and the server stops accepting new clients
            // after a small number of requests.
            _tcp_backlog_accepted(_pcb);
